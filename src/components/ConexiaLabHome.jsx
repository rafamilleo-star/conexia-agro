...
function PersonMini({ person }) {
  const initials = String(person?.name || "?").split(/\s+/).map(x => x[0]).join("").slice(0,2).toUpperCase();
  return (
    <div style={{ minWidth: 120, textAlign: "center" }}>
      <div style={{
        width: 54, height: 54, borderRadius: "50%", margin: "0 auto 7px",
        border: `1px solid ${K.gold}80`, background: K.card2,
        display: "flex", alignItems: "center", justifyContent: "center",
        color: K.gold, fontFamily: sans, fontWeight: 800
      }}>{initials}</div>
      <div style={{ color: K.text, fontFamily: sans, fontSize: 13, fontWeight: 800 }}>{person?.name || "Pessoa"}</div>
...
function ConnectionView({ people = [], topics = [], answer, onWhy, onCreateBridge, onTomorrow }) {
  ...
  return (
    <div>
      <div style={{ color: K.gold, fontFamily: sans, fontSize: 10, fontWeight: 800, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 10 }}>
        POSSÍVEL PONTE
      </div>

      <div style={{
        display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center",
        background: K.card2, border: `1px solid ${K.gold}40`, borderRadius: 16, padding: 16, gap: 10
      }}>
        <PersonMini person={a} />
        <div style={{ textAlign: "center", minWidth: 110 }}>
          <div style={{ height: 1, background: `linear-gradient(90deg, transparent, ${K.gold}, transparent)`, marginBottom: 8 }} />
          <div style={{ color: K.gold, fontFamily: sans, fontSize: 10, fontWeight: 800 }}>CONEXÃO POR</div>
          <div style={{ display: "flex", gap: 5, justifyContent: "center", flexWrap: "wrap", marginTop: 6 }}>
            {(topics.length ? topics : ["contexto"]).slice(0,3).map(t => (
              <span key={t} style={{ border: `1px solid ${K.gold}50`, borderRadius: 999, padding: "4px 7px", color: K.text, fontFamily: sans, fontSize: 10 }}>{t}</span>
            ))}
          </div>
        </div>
        <PersonMini person={b} />
      ...
      {answer && <div style={{ color: K.text, fontFamily: sans, fontSize: 13.5, lineHeight: 1.6, marginTop: 14 }}>{answer}</div>}
...
function CaptureView({ draft, onConfirm, onCorrect }) {
  return (
    <div>
      <div style={{ color: K.gold, fontFamily: sans, fontSize: 10, fontWeight: 800, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 9 }}>
        ENTENDI ISTO
      </div>
      <div style={{ color: K.text, fontFamily: serif, fontSize: 28, fontWeight: 700, marginBottom: 7 }}>
        {draft?.contactName || "Nova interação"}
      </div>
      <div style={{ color: K.muted, fontFamily: sans, fontSize: 13, lineHeight: 1.6 }}>{draft?.description}</div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 14 }}>
        ...
        ].filter(Boolean).map(v => <span key={v} style={chip}>{v}</span>)}
      </div>

      <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
        <button onClick={onConfirm} style={{ ...primaryButton, background: K.green, color: "#0A0D0B" }}>Confirmar e salvar</button>
...
function AnswerView({ question, answer }) {
  return (
    <div>
      {question && (
        <div style={{
          marginLeft: "auto", maxWidth: "82%", background: K.card2, border: `1px solid ${K.border}`,
          borderRadius: "14px 14px 4px 14px", padding: "10px 12px",
          color: K.muted, fontFamily: sans, fontSize: 12.5, lineHeight: 1.5, marginBottom: 12
        }}>{question}</div>
      )}
      <div style={{
        maxWidth: "92%", background: `${K.gold}0D`, border: `1px solid ${K.gold}35`,
        borderRadius: "14px 14px 14px 4px", padding: "14px 15px"
      }}>
        <div style={{ color: K.gold, fontFamily: sans, fontSize: 9, fontWeight: 800, letterSpacing: ".1em", marginBottom: 5 }}>CONÉXIA</div>
        <div style={{ color: K.text, fontFamily: sans, fontSize: 14, lineHeight: 1.65 }}>{answer}</div>
      </div>
    </div>
  );
}
...
function InsightView({ title, body, onOpen }) {
  return (
    <div>
      <div style={{ color: K.gold, fontFamily: sans, fontSize: 10, fontWeight: 800, letterSpacing: ".12em", textTransform: "uppercase", marginBottom: 8 }}>OLHE ISSO</div>
      <div style={{ color: K.text, fontFamily: serif, fontSize: 27, fontWeight: 700 }}>{title}</div>
      <div style={{ color: K.muted, fontFamily: sans, fontSize: 13, lineHeight: 1.6, marginTop: 7 }}>{body}</div>
      {onOpen && <button onClick={onOpen} style={{ ...secondaryButton, marginTop: 13 }}>Abrir pessoa</button>}
    </div>
  );
}
...
export default function ConexiaLabHome({
  ...
  const [currentView, setCurrentView] = useState("today"); // today | answer | connection | capture | saved
  ...
  const todayInsight = useMemo(() => {
    ...
    return {
      ...
      body: "Você pode conversar comigo, registrar algo que aconteceu ou perguntar sobre alguém da sua rede.",
    };
  ...
  const startListening = () => {
    ...
    if (!SR) {
      setError("Este navegador não liberou reconhecimento de voz. Use o campo de texto abaixo.");
      setVoiceState("idle");
      return;
    }
    ...
    rec.onend = () => {
      ...
      if (spoken) {
        ...
        setVoiceState("idle");
        setTimeout(() => {
          if (conversationActiveRef.current && !recRef.current) startListening();
        }, 650);
      }
    };
    rec.start();
  ...
  const findContactByName = (name) => {
    ...
    if (!n) return null;
    return contacts.find(c => normalize(c.name) === n)
      || contacts.find(c => normalize(c.name).includes(n) || n.includes(normalize(c.name)));
  };

  const analyzeCapture = async (text) => {
    const existing = contacts.map(c => ({ id:c.id, name:c.name, company:c.company || "", role:c.role || "" }));
    const prompt = `Você é o motor de captura do CONÉXIA. Extraia apenas o que está explícito, sem inventar.
...
Responda SOMENTE JSON válido:
{
  ...
  "nextActionDate": "YYYY-MM-DD apenas se houver segurança ou null"
    ...
    if (!parsed) throw new Error("Não consegui estruturar a interação.");
    ...
    sessionContextRef.current.activePerson = parsed.contactName || null;
    sessionContextRef.current.lastView = "capture";
    speak(`Entendi. ${parsed.contactName ? `É sobre ${parsed.contactName}. ` : ""}${parsed.description || ""} Quer que eu salve?`, true);
  };
  ...
  const askNetwork = async (text) => {
    const people = contacts.map(c => ({
      ...
      category:c.category || "",
      city:c.city || "",
      notes:(c.personal_notes || c.notes || "").slice(0,260),
    ...
    const prompt = `Você é o CONÉXIA, inteligência relacional. Responda somente com base nos dados fornecidos. Não invente.
...
Responda SOMENTE JSON válido:
{
  "answer":"resposta natural, curta, no máximo 5 frases",
  "view":"answer|connection|person|insight",
  "people":["nomes exatos de até 2 pessoas relevantes"],
  "topics":["até 3 temas que justificam a leitura"],
  "suggestedActions":["why","bridge","tomorrow"] 
}

Use "connection" quando houver uma conexão concreta entre duas pessoas.
Use "person" quando a pergunta for essencialmente sobre uma pessoa.
Use "insight" quando for um padrão/provocação sobre a rede.
Use "answer" nos demais casos.`;
    ...
    setAnswer(parsed.answer || "Não encontrei evidência suficiente.");
    ...
    speak(parsed.answer || "Não encontrei evidência suficiente para responder.", true);
  };
  ...
  const handleUserTurn = async (text) => {
    ...
    if (currentView === "capture" && draft) {
      if (/^(sim|pode|salva|salvar|confirma|confirmar|correto|isso|certo)/.test(n)) {
      ...
      if (/^(nao|não|corrige|corrigir|cancela|cancelar)/.test(n)) {
        ...
        speak("Tudo bem. Me conta novamente do jeito que aconteceu.", true);
        return;
      }
    }

    try {
      const prompt = `Classifique a intenção da fala no CONÉXIA.
...
{"intent":"capture|ask|chat"}

capture = contando interação, encontro, ligação, mensagem, promessa, contexto de alguém.
ask = pergunta, busca, análise, recomendação, relação, conexão ou próximo movimento.
chat = saudação ou conversa curta sem pedido relacional.`;
      ...
      if (parsed.intent === "capture") {
        ...
        setAnswer(`${greetingForNow(displayName)} Pode falar. Eu estou com sua rede aqui.`);
        setCurrentView("answer");
        speak(`${greetingForNow(displayName)} Pode falar. Eu estou com sua rede aqui.`, true);
      }
    } catch (e) {
    ...
    try {
      ...
      if (!contactId) {
        const { data, error } = await supabase.from("contacts").insert({
          ...
          status:"active",
        }).select("id,name,company,role").single();
      ...
      if (draft.nextAction) patch.next_action = draft.nextAction;
      if (draft.nextActionDate) patch.next_action_date = draft.nextActionDate;
      if (!contact?.company && draft.company) patch.company = draft.company;
      if (!contact?.role && draft.role) patch.role = draft.role;
      ...
      setDraft(null);
      setAnswer(`Registrei com ${who}.`);
      setCurrentView("saved");
      await onDataChanged?.();
      speak(`Pronto. Registrei com ${who}.`, true);
    } catch (e) {
  ...
  const createBridge = () => {
    ...
    setAnswer(`A melhor próxima ação é você fazer a ponte entre ${a.name} e ${b.name}. Posso deixar isso como próximo movimento.`);
    setCurrentView("answer");
    speak(`A melhor próxima ação é você fazer a ponte entre ${a.name} e ${b.name}. Posso deixar isso como próximo movimento.`, true);
  };
  ...
  const remindTomorrow = async () => {
    ...
    const action = other ? `Apresentar ${person.name} a ${other}` : `Retomar ${person.name}`;
    ...
    setAnswer(`Fechado. Deixei ${action.toLowerCase()} para amanhã.`);
    setCurrentView("answer");
    await onDataChanged?.();
  ...
  if (prefsLoading) {
    return <div style={{ color:K.text, fontFamily:sans, padding:30 }}>Preparando CONÉXIA Live...</div>;
  }

  const greeting = prefs?.greeting_mode === "direto"
    ? `${displayName ? `${displayName}, ` : ""}vamos cuidar da sua rede.`
    : greetingForNow(displayName);

  return (
    <div style={{ maxWidth: 820, margin: "0 auto", paddingBottom: 42 }}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ color:K.gold, fontFamily:sans, fontSize:10, fontWeight:800, letterSpacing:".14em", textTransform:"uppercase", marginBottom:7 }}>
          CONÉXIA LIVE
        </div>
        <h1 style={{ color:K.text, fontFamily:serif, fontSize:32, lineHeight:1.05, margin:"0 0 7px", fontWeight:700 }}>
          {greeting}
        </h1>
        <div style={{ color:K.muted, fontFamily:sans, fontSize:13 }}>
          {currentView === "today" ? "Tem uma coisa na sua rede que eu olharia hoje." : "Eu acompanho o contexto enquanto você conversa."}
        </div>
      </div>

      <div style={{
        background:K.card, border:`1px solid ${K.border}`, borderRadius:22, padding:20,
        boxShadow:"0 16px 42px rgba(0,0,0,.20)"
      }}>
        {currentView === "today" && (
          <InsightView title={todayInsight.title} body={todayInsight.body} onOpen={todayInsight.onOpen} />
        )}
        ...
        {currentView === "saved" && (
          <div style={{ textAlign:"center" }}>
            <div style={{ width:56, height:56, borderRadius:"50%", background:`${K.green}18`, border:`1px solid ${K.green}55`, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px", color:K.green, fontSize:26 }}>✓</div>
            <div style={{ color:K.text, fontFamily:serif, fontSize:27, fontWeight:700 }}>Registrado.</div>
            <div style={{ color:K.muted, fontFamily:sans, fontSize:13, marginTop:6 }}>{answer}</div>
          </div>
        )}

        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginTop:22 }}>
          <VoiceOrb state={voiceState} active={conversationActive} onClick={beginConversation} />
          <div style={{ color:K.muted, fontFamily:sans, fontSize:10.5, marginTop:10 }}>
            {conversationActive
              ? "Sessão aberta: fale normalmente. O CONÉXIA volta a ouvir depois de responder."
              : "Toque uma vez para conversar."}
          </div>

          {input && voiceState === "listening" && (
            <div style={{ marginTop:12, color:K.text, fontFamily:sans, fontSize:12.5, opacity:.85 }}>
              “{input}”
            </div>
          )}
          ...
          <button
            ...
            style={{ marginTop:10, background:"transparent", border:"none", color:K.muted, fontFamily:sans, fontSize:11, cursor:"pointer", textDecoration:"underline" }}
          >
        ...
        {showText && (
          <div style={{ marginTop:16, paddingTop:16, borderTop:`1px solid ${K.border}` }}>
            <textarea
              value={textInput}
              ...
              style={{
                ...
                background:K.card2, border:`1px solid ${K.border}`, borderRadius:12,
                color:K.text, padding:13, fontFamily:sans, fontSize:13.5, lineHeight:1.5, outline:"none"
              }}
            />
            <div style={{ display:"flex", justifyContent:"flex-end", marginTop:8 }}>
              <button onClick={submitText} style={primaryButton}>Enviar</button>
            </div>
          </div>
        )}

        {error && (
          <div style={{ marginTop:14, color:K.red, fontFamily:sans, fontSize:12, lineHeight:1.5 }}>
            {error}
          </div>
        )}
      </div>

      <div style={{ marginTop:10, display:"flex", justifyContent:"center", gap:24, color:K.muted, fontFamily:sans, fontSize:10 }}>
        <span>Hoje</span>
        <span>Rede</span>
        <span style={{ color:K.gold, fontWeight:800 }}>Conversar</span>
        <span>Eu</span>
      </div>
    </div>
  );
